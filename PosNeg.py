    ...
    with open('D://stars_reviews.txt',"r") as loadfp:
        ...
        for row in loadfp.readlines():
            ...
            if stars_split and review_split:
                  if(stars_split=='3' or stars_split=='4' or stars_split=='5'):
            ...
            count = count+1
            if(count >25000): #To specify the number of Tweets
                break
...
def preprocess(data,target):
    ...
    count_vectorizerNB = CountVectorizer(binary='false',ngram_range=(1
                                                                     ,1))
...
def learn_model(dataSVM,dataNB, target):
    ...
    data_train, data_test, target_train, target_test = cross_validation.train_test_split(dataNB, target, test_size=0.3,
                                                                                         random_state=37)
    calculateMajorityClass(target_train)

    print "Using Bernoulli's Naive Bayes"
    bm= BernoulliNB(fit_prior=True)
